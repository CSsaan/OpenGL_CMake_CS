...
public:
    ...
    void srtUniformMat2x4(const std::string &name, const glm::mat2x4 &mat)
    {
        GLint have_uniform = glGetUniformLocation(ID, name.c_str());
        checkUniformLocation(have_uniform, name);
...
private:
    // ------------------------------------------------------------------------
    void checkCompileErrors(unsigned int shader_program, std::string type)
    {
        int success;
        char infoLog[1024];
    ...
    // ------------------------------------------------------------------------
    void checkUniformLocation(GLint have_uniform, const std::string &name)
    {
        if(have_uniform == -1)
        {
